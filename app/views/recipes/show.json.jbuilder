...
json.ingredients @recipe.ingredients.inject({}) {|ings, i|
  ...
  if i.food
    ings[i.id] = {ml: qty.ml, grams: qty.grams, total: qty.total, unit_weight: i.food.unit_weight, density: i.food.density, food_id: i.food.id, food_name: i.food.name, food_plural: i.food.plural, raw: i.raw, comment: i.comment}
  else
    ings[i.id] = {ml: qty.ml, grams: qty.grams, total: qty.total, raw: i.raw, comment: i.comment}
  end
  ings
}
