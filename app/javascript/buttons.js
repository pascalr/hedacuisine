      ...
      {!gon.recipe ? null : <li>
        ...
        <ul className="dropdown-menu dropdown-submenu">
          {Object.values(gon.recipe.notes || {}).map(note => (
            <li key={note.id}>
              <a className="dropdown-item" onClick={() => editor.chain().focus().insertLinkModel('note', note.item_nb).run()}>[{note.item_nb}] {Utils.stringSnippet(Utils.stripHtml(note.content))}</a>
